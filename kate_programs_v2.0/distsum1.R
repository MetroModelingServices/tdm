# distsum1.R - creates 8-district summary - WRS 12/04/02
# adapted from collapse.v2 by Brian Gregor (ODOT)

distsum1 <- function (mat, desc, ens, digits, rpt, proj, init) {
...
# example:  distsum1 ("mf.colldt", "College Distribution", "ga", 3,
...
# Print header lines
  outfile <- file ("tmpfile1", "w")
#  if (length (system (paste ("ls ", rpt, ".rpt", sep=''),TRUE,TRUE)) == 1) {
...
# Process full matrix
  if (type == "mf") {
    ...
    options (warn = -1)
    write.table (round (dist.sum, digits), "tmpfile1", append=T,
...
#    options (warn = 1)
    ...
    if (file.exists("tmpfile1") == 1) {system ("rm tmpfile1")}
  }
...
#    options (warn = 0)
    if(.Platform$OS=="unix"){
    system (paste ("awk -f ", R.path,"/src/awk/distsum.awk",
      " -v numdist=1 tmpfile1 >> ", rpt, ".rpt", sep=''))
    }
    if(.Platform$OS=="windows"){
    shell (paste ("gawk -f ", R.path,"/src/awk/distsum.awk",
      " -v numdist=1 tmpfile1 >> ", rpt, ".rpt", sep=''))
